...
def apply_crm(df: pd.DataFrame, crm_config: dict) -> pd.DataFrame:
    ...
    out["crm_eligible"] = out["collateral_type"].map(lambda x: bool(rules.get(x, rules["none"])["eligible"]))
    out["collateral_haircut"] = out["collateral_type"].map(lambda x: float(rules.get(x, rules["none"])["haircut"]))
    out["crm_treatment"] = out["collateral_type"].map(lambda x: rules.get(x, rules["none"])["treatment"])
    ...
    out["sa_ead_post_crm"] = np.maximum(out["ead_pre_crm"] - out["collateral_exposure_reduction"], 0.0)
    ...
    out["crm_coverage_ratio"] = np.where(
        out["ead_pre_crm"] > 0,
        np.minimum((out["eligible_collateral_value"] + out["eligible_guarantee_value"]) / out["ead_pre_crm"], 1.0),
        0.0,
    )
    coverage = np.minimum(out["eligible_collateral_value"] / out["ead_pre_crm"].replace(0, np.nan), 1.0).fillna(0)
    out["lgd_after_crm"] = np.maximum(out["lgd"] * (1 - 0.70 * coverage), 0.05)
    return out
