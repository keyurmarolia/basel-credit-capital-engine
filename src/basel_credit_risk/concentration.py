...
def concentration_by(df: pd.DataFrame, dimension: str) -> pd.DataFrame:
    ...
    pd_weight = (
        df.assign(pd_ead=df["pd_regulatory"] * df["ead_pre_crm"])
        .groupby(dimension, dropna=False)["pd_ead"]
        .sum()
    )
    lgd_weight = (
        df.assign(lgd_ead=df["lgd_regulatory"] * df["ead_pre_crm"])
        .groupby(dimension, dropna=False)["lgd_ead"]
        .sum()
    )
...
def borrower_concentration(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = (
        ...
        .agg(
            ead=("ead_pre_crm", "sum"),
            irb_rwa=("irb_rwa", "sum"),
            facility_count=("exposure_id", "count"),
        )
