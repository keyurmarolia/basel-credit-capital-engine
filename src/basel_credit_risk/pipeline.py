...
def run_pipeline(
    ...
    synthetic: SyntheticPortfolio = generate_portfolio(
        size=portfolio_size or int(scope["portfolio_size"]),
    ...
    for scenario_name, scenario_cfg in configs["stress_scenarios"]["scenarios"].items():
        ...
        floor = calculate_output_floor(frame["sa_rwa"].sum(), frame["irb_rwa"].sum(), market_rwa, operational_rwa, floor_rate)
        ...
        capital_tables.append(calculate_capital_adequacy(scenario_name, final_credit, market_rwa, operational_rwa, stress_loss, cap_config))
        summary_rows.append(
            {
    ...
    rating = current.groupby("external_rating", as_index=False).agg(ead=("ead_pre_crm", "sum"), sa_rwa=("sa_rwa", "sum"), irb_rwa=("irb_rwa", "sum"), weighted_pd=("pd_regulatory", "mean"))
    ...
    tables = {
        ...
        "sa_summary": current.groupby("basel_exposure_class", as_index=False).agg(ead_pre_crm=("ead_pre_crm", "sum"), ead_post_crm=("sa_ead_post_crm", "sum"), sa_rwa=("sa_rwa", "sum"), average_risk_weight=("sa_risk_weight", "mean")),
        "irb_summary": current.groupby("basel_exposure_class", as_index=False).agg(ead=("ead_irb", "sum"), weighted_pd=("pd_regulatory", "mean"), weighted_lgd=("lgd_regulatory", "mean"), expected_loss=("expected_loss_amount", "sum"), irb_rwa=("irb_rwa", "sum")),
        ...
        "rwa_bridge": attribute_rwa(prior, current),
        "scenario_summary": pd.DataFrame(summary_rows),
        "stress_sector_contribution_adverse": stress_contribution(scenario_frames["base"], scenario_frames["adverse"], "sector"),
        "stress_sector_contribution_severe": stress_contribution(scenario_frames["base"], scenario_frames["severe"], "sector"),
        "output_floor": pd.concat(floor_tables, ignore_index=True),
        "capital_adequacy": pd.concat(capital_tables, ignore_index=True),
        "exposure_trace_sample": current.head(25),
    }

    if write_outputs:
        data_generated = root / "data" / "generated"
        data_generated.mkdir(parents=True, exist_ok=True)
        synthetic.current.to_csv(data_generated / "current_portfolio.csv.gz", index=False, compression="gzip")
        synthetic.prior.to_csv(data_generated / "prior_portfolio.csv.gz", index=False, compression="gzip")
        ...
        for scenario_name, frame in scenario_frames.items():
            frame.to_csv(processed / f"scenario_{scenario_name}.csv.gz", index=False, compression="gzip")
        sample = root / "data" / "sample"
        sample.mkdir(parents=True, exist_ok=True)
        synthetic.current.head(500).to_csv(sample / "synthetic_exposure_sample.csv", index=False)
